...
class CorpusReader:
    ...
    def __init__(self, src_dirname, metadata_filename):
        """
...
class Utterance:
    ...
    'turn_index':     (int) The encoded index of the turn (the number in A.49, B.27, etc.)
    ...
    'start_turn':          (float) The start time of the turn to which the utterance belongs
    'end_turn':            (float) The end time of the turn to which the utterance belongs
    """
    ...
    header = [
        ...
        'act_tag',            # (list of str) The Dialog Act Tags (separated by ||| in the file).
        'caller',             # (str) A, B, @A, @B, @@A, @@B
        'turn_index',         # (int) The encoded index of the utterance (the number in A.49, B.27, etc.)
        'subutterance_index', # (int) Utterances can be broken across line. This gives the internal position.
        'text',               # (str) The text of the utterance
        'pos',                # (str) The POS tagged version of the utterance, from PtbBasename+.pos
        'trees',              # (list of nltk.tree.Tree) The tree(s) containing this utterance (separated by ||| in the file).
    ...
    def __init__(self, row, transcript_metadata):
        ...
        for i in xrange(len(Utterance.header)):
            ...
            if att_name == "trees":
                if row_value: row_value = map(Tree.fromstring, row_value.split("|||"))
                else: row_value = []
            ...
            elif att_name in ('conversation_no', 'transcript_index', 'turn_index', 'subutterance_index'):
                row_value = int(row_value)                
            elif att_name in ('start_turn', 'end_turn'):
                try:
                    row_value = float(row_value)

                # ignore missing values TODO is this right this way?
                except ValueError:
                    pass
                except TypeError:
                    pass
